		...
		<ScrollView style={{ backgroundColor: palette.background }} contentContainerStyle={styles.content}>
			...
			{tagPool.length > 0 ? (
				<View style={styles.tagRow}>
					<PressableScale onPress={() => setSelectedTag(null)} contentStyle={[styles.tagButton, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
						<Text style={[styles.tagLabel, { color: palette.text }]}>All tags</Text>
					</PressableScale>
					{tagPool.map((tag) => (
						<PressableScale key={tag} onPress={() => setSelectedTag(tag === selectedTag ? null : tag)} contentStyle={[styles.tagButton, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
			...
			<PressableScale onPress={() => router.push('/card/new' as never)} contentStyle={[styles.button, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
			...
			{filteredCards.map((card) => {
				...
				return (
					<PressableScale key={card.id} onPress={() => router.push(`/card/${card.id}` as never)} contentStyle={[styles.section, { backgroundColor: palette.card, borderColor: palette.border }]}> 
						...
						<Text style={[styles.helper, { color: palette.muted }]}>
							{nextReminder ? `Next reminder: ${nextReminder.title} · ${formatFullDate(nextReminder.scheduledFor)}` : 'No active reminders right now.'}
						</Text>
						{card.tags.length > 0 ? <Text style={[styles.helper, { color: palette.muted }]}>Tags: {card.tags.join(', ')}</Text> : null}
					</PressableScale>
				);
			})}
		</ScrollView>
