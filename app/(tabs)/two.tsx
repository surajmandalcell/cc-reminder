...
export default function CardsScreen() {
  ...
  const filteredCards = useMemo(() => {
    ...
    return cards.filter((card) => {
      const matchesSearch =
        !searchTerm ||
        [card.name, card.provider, card.last4, ...card.tags].join(' ').toLowerCase().includes(searchTerm);
  ...
  return (
    <ScrollView style={{ backgroundColor: palette.background }} contentContainerStyle={styles.content}>
      <View style={[styles.hero, shadow.medium, { backgroundColor: palette.card, borderColor: palette.border, shadowColor: palette.glow }]}> 
        ...
        <Text style={[styles.title, { color: palette.text }]}>Cards stay minimal, but the view should still feel premium.</Text>
        <Text style={[styles.body, { color: palette.muted }]}>Track providers, last 4 digits, due dates, tags, reminder settings, and optional extended follow-up without ever storing sensitive payment credentials.</Text>
      </View>
      ...
      {tagPool.length > 0 ? (
        <View style={styles.tagRow}>
          <PressableScale onPress={() => setSelectedTag(null)} contentStyle={[styles.tagChip, { backgroundColor: selectedTag ? palette.card : palette.text, borderColor: selectedTag ? palette.border : palette.text }]}> 
          ...
          {tagPool.map((tag) => (
            <PressableScale
              ...
              contentStyle={[styles.tagChip, { backgroundColor: selectedTag === tag ? palette.text : palette.card, borderColor: selectedTag === tag ? palette.text : palette.border }]}> 
      ...
      {filteredCards.map((card) => {
        ...
        return (
          <PressableScale key={card.id} onPress={() => router.push(`/card/${card.id}` as never)} contentStyle={[styles.card, shadow.soft, { backgroundColor: palette.card, borderColor: palette.border, shadowColor: palette.glow }]}> 
