	...
	return (
		<ScrollView style={{ backgroundColor: palette.background }} contentContainerStyle={styles.content}>
			...
			{reminders.map((reminder) => {
				...
				return (
					<View key={reminder.id} style={[styles.section, { backgroundColor: palette.card, borderColor: palette.border }]}> 
						...
						<View style={styles.actions}>
							<PressableScale onPress={() => void acknowledgeReminder(reminder.cardId, reminder.id)} contentStyle={[styles.button, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
								<Text style={[styles.buttonLabel, { color: palette.text }]}>{reminder.isAcknowledged ? 'Acknowledged' : 'Acknowledge'}</Text>
							</PressableScale>
							<PressableScale onPress={() => void snoozeReminder(reminder.cardId, reminder.id, 12)} contentStyle={[styles.button, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
								<Text style={[styles.buttonLabel, { color: palette.text }]}>Snooze 12h</Text>
							</PressableScale>
							<PressableScale onPress={() => void settleCycle(reminder.cardId, reminder.cycleId)} contentStyle={[styles.button, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
								<Text style={[styles.buttonLabel, { color: palette.text }]}>{reminder.isSettled ? 'Settled' : 'Settle cycle'}</Text>
							</PressableScale>
							<PressableScale onPress={() => router.push(`/card/${reminder.cardId}` as never)} contentStyle={[styles.button, { backgroundColor: palette.cardAlt, borderColor: palette.border }]}> 
